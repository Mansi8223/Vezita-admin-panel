...
function DoctorsTable({data,searchTerm}) {
    return (
          ...
          <Table size="small" aria-label="a dense table" >
            <TableHead className={`bg-secondary`}>
              <TableRow className={`table-header`}>
                ...
                <TableCell align="left">No. of patient</TableCell>
                <TableCell align="left">Session charges</TableCell>
            ...
            if(searchTerm == ""){
              ...
              return val
                ...
                <TableRow key={index+1} className={`table-body`}>
                  ...
                  <TableCell align="left">{item.numOfPatientConsulted}</TableCell>
                  {item.establishment&&<TableCell align="left">${item.establishment[0].consultationFee}</TableCell>}
                  {item.status==="pending"&&<TableCell align="left"></TableCell>}
                  {item.status==="block"&&<TableCell align="left"><h6 className={`f-500 l-20`}>{item.status}</h6></TableCell>}
                  {item.status==="active"&&<TableCell align="left"><h6 className={`f-500 l-20 text-green`}>{item.status}</h6></TableCell>}
                  {item.status==="inactive"&&<TableCell align="left"><h6 className={`f-500 l-20 text-red`}>{item.status}</h6></TableCell>}
                  <TableCell align="left">{item.docterRevenue}</TableCell>
