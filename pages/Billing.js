...
function Billing() {
  ...
  const[loading, setLoading]=useState(false)
    useEffect(()=>{
      ...
      fetch("https://vezita-backend.herokuapp.com/api/v1/docter-analytics/docter-revenue-admin", requestOptions)
        .then(response => response.text())
        .then(result => {
          var res = JSON.parse(result);
          console.log(res.docterRevenue)
          setData(res.docterRevenue)
          setLoading(false)
    ...
    <>
      {loading?<Loader loading={loading}/>:<Fragment>
        <Base>
          ...
          <BillingTable data={data} searchTerm={searchTerm}/>
        </Base>
      </Fragment>}
    </>
  )
