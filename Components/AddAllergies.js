...
import AddAllergyContent from './Modals/AddAllergyContent';
...
function AddAllergies() {
        ...
        var requestOptions = {
          ...
          fetch("https://vezita-backend.herokuapp.com/api/v1/medical-data/allergy", requestOptions)
            ...
            .then(result => {
                ...
                // console.log(res.allergyData)
                setData(res.allergyData)
                setLoading(false)
            })
            .catch(error =>{ 
    ...
    const deleteHandler=()=>{
        ...
        fetch(`https://vezita-backend.herokuapp.com/api/v1/medical-data/allergy/${Id}`, requestOptions)
        .then(response => response.text())
        .then(result => {
            setLoading(true)
            var myHeaders = new Headers();
            ...
            {loading?<Loader loading={loading}/>:<>
                <div className={`col-12 h-fit-content d-flex d-flex-column d-align-start gap-8 bg-subtle rounded-12 p-4`}>
                    <div className={`col-12 d-flex d-flex-row d-justify-space-between d-align-center`}>
                        <h3 className={`f-500 l-28 text-black`}>Allergies</h3>
                ...
                {open&&<AddAllergyModal modalClass="modal-verify">
                    <AddAllergyContent handler={handler} trigger={triggerHandler}></AddAllergyContent>
                </AddAllergyModal>}
            </>}
        </>
    )
