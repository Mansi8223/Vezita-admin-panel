...
import AddSpecializationContent from './Modals/AddSpecializationContent';
...
function AddSpecialization() {
   ...
   const deleteHandler=()=>{
    ...
    fetch(`https://vezita-backend.herokuapp.com/api/v1/service/specialization/${Id}`, requestOptions)
    .then(response => response.text())
    .then(result => {
        // console.log(result)
        setLoading(true)
        ...
        {loading?<Loader loading={loading}/>:<>
            <div className={`col-12 h-fit-content d-flex d-flex-column d-align-start gap-8 bg-subtle rounded-12 p-4`}>
                <div className={`col-12 d-flex d-flex-row d-justify-space-between d-align-center`}>
                    <h3 className={`f-500 l-28 text-black`}>Specialization</h3>
            ...
            {open&&<AddSpecializationModal modalClass="modal-verify">
                <AddSpecializationContent handler={handler} trigger={triggerHandler}></AddSpecializationContent>
