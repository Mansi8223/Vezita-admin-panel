...
import AddDrugModalContent from './Modals/AddDrugModalContent';
...
function AddDrugs() {
    ...
    useEffect(()=>{
        ...
        var requestOptions = {
          ...
          fetch("https://vezita-backend.herokuapp.com/api/v1/medical-data/drug", requestOptions)
            .then(response => response.text())
            .then(result => {
                var res = JSON.parse(result);
                setData(res.drugData)
                setLoading(false)
            })
            .catch(error =>{ 
    ...
    const deleteHandler=()=>{
        ...
        fetch(`https://vezita-backend.herokuapp.com/api/v1/medical-data/drug/${Id}`, requestOptions)
        .then(response => response.text())
        .then(result => {
            setLoading(true)
            var myHeaders = new Headers();
            ...
            {loading?<Loader loading={loading}/>:<>
                <div className={`col-12 h-fit-content d-flex d-flex-column d-align-start gap-8 bg-subtle rounded-12 p-4`}>
                    <div className={`col-12 d-flex d-flex-row d-justify-space-between d-align-center`}>
                        <h3 className={`f-500 l-28 text-black`}>Drugs</h3>
                    ...
                    <div className={`col-6 col-md-6 col-lg-5 col-xxl-6 d-flex d-flex-column d-align-start gap-3`}>
                        ...
                        {data && data.filter((val)=>{
                            ...
                            }).map((item,index)=>(<div key={index+1} className={`col-12 d-flex d-flex-row d-align-center gap-3`}>
                ...
                {open&&<AddDrugModal modalClass="modal-verify">
                    <AddDrugModalContent handler={handler} trigger={triggerHandler}></AddDrugModalContent>
                    </AddDrugModal>}
            </>}
        </>
    )
