...
function ViewConsultations() {
    ...
    fetch(`https://vezita-backend.herokuapp.com/api/v1/doctor-patient?docter=${Id}`, requestOptions)
      .then(response => response.text())
      .then(result => {
        var res = JSON.parse(result);
        setData(res.docterPatient)
        setLoading(false)
      })
      .catch(error => {
  ...
  return (
    <>
      {loading?<Loader loading={loading}/>:<Fragment>
        <Base>
          <Header title={"Consultations"}></Header>
          ...
          <Consultations data={data} searchTerm={searchTerm}/>
        </Base>
      </Fragment>}
    </>
  )
